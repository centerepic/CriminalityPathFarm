...
__newindex = hookmetamethod(game, "__newindex", function(t, k, v)
    ...
    if (not checkcaller() and t.Name == "Sprinting" and t:IsA("BoolValue") and (k == "Value")) then
        return
    end
...
for i,v in pairs(workspace:GetDescendants()) do
    ...
    if v:IsDescendantOf(workspace.Map.Doors) and v:IsA("BasePart") then
        v.CanCollide = false
    end
...
ReplicatedStorage.CharStats[LocalPlayer.Name].Sprinting.Value = true
...
local BreakAnim = Instance.new("Animation")
BreakAnim.AnimationId = "rbxassetid://7585781452"

local BreakAnimFist = Instance.new("Animation")
BreakAnimFist.AnimationId = "rbxassetid://5656642345"

local WalkTrack = LocalPlayer.Character.Humanoid:LoadAnimation(LocalPlayer.Character.Animate.walk1.WalkAnim1)
local RunTrack = LocalPlayer.Character.Humanoid:LoadAnimation(LocalPlayer.Character.Animate.run1.RunAnim1)
local PickupTrack = LocalPlayer.Character.Humanoid:LoadAnimation(game:GetService("ReplicatedStorage").Storage.Animations.Pickup1)
local BreakTrack = LocalPlayer.Character.Humanoid:LoadAnimation(BreakAnim)
local BreakTrackFist = LocalPlayer.Character.Humanoid:LoadAnimation(BreakAnimFist)
        ...
        if TargetSafe.Name:find("Register") then
            if LocalPlayer.Character:FindFirstChild("Fists") then
                ...
                if ClosestRegister and not AutoBreakRegisterCoolDown then
                    ...
                    TTP(CFrame.new((TargetSafe.PosPart.CFrame - TargetSafe.PosPart.CFrame.LookVector * 2).Position,TargetSafe.MainPart.Position))
                    BreakTrackFist:Play(0.1,1,1)
                    wait(BreakTrackFist.Length - 0.3)
